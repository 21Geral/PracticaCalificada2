...
function Search({ setSearchText, setCategories }) {
  const [inputValue, setInputValue] = useState("");

  const handleSubmit = (e) => {
    e.preventDefault();

    setCategories((prev) => prev.map((cat) => ({ ...cat, status: false })));

    setSearchText(inputValue);
  };

  return (
    <form className="relative w-full md:w-120" onSubmit={handleSubmit}>
      <input
        type="text"
        value={inputValue}
        onChange={(e) => setInputValue(e.target.value)}
        className="w-[90%] outline-none py-2 px-4 z-20 text-[16px] md:text-xl text-gray-900 bg-[#edeff0] rounded-s-md
             border-r-[#edeff0] border-r-3 border-3 border-gray-300   "
        placeholder="Search..."
      />
      <button
        type="submit"
        className={`
                    ...
                    ${inputValue ? "bg-[#051c33] cursor-pinter" : "bg-gray-100 cursor-not-allowed"}
                `}
        disabled={!inputValue}
      >
